...
const AboutCard = ({ card }) => {
  ...
  const preview = card.content.length > 220 ? card.content.slice(0, 220) + '...' : card.content;
  ...
  return (
    <div className="bg-white rounded-lg shadow-md p-4 border border-amber-100 flex flex-col">
      ...
      {card.content.length > 220 && (
        <div className="mt-3">
          <button onClick={() => setShowModal(true)} className="text-amber-600 hover:underline">
            Learn more
          </button>
        </div>
      )}
      ...
      {showModal && (
        <div onClick={() => setShowModal(false)} className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4">
          <div onClick={(e) => e.stopPropagation()} className="bg-white max-w-3xl w-full max-h-[80vh] overflow-y-auto rounded-lg p-6 relative">
            <button
