...
/**
 * Brochure Image Schema (embedded)
...
/**
 * Brochure Category Schema
 * For folder-based gallery (Brochure section)
 *
 * IMPORTANT:
...
brochureCategorySchema.virtual("imageCount").get(function () {
  return this.images ? this.images.filter((img) => img.isVisible).length : 0;
});

// Enable virtuals
brochureCategorySchema.set("toJSON", { virtuals: true });
brochureCategorySchema.set("toObject", { virtuals: true });
